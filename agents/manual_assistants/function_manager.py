...
class FunctionManager:
    ...
    def __init__(self, additional_functions=None):
        self.additional_functions = additional_functions or {}
        self.log = Logger(self.__class__.__name__)
        self.user_function_dirs = (
            util.get_environment_variable_list("function_dir") or []
        )
        self.make_user_function_dirs()
        self.system_function_dirs = [
            os.path.join(util.get_file_directory(), "agentTools", "functions"),
        ]
    ...
    def setup_function_instance(self, function_name, function_path, context=None, agent=None, execution=None):
        ...
        try:
            ...
            function_instance.set_context(context)
            function_instance.set_agent(agent)
            function_instance.set_execution(execution)
    ...
    def run_function(self, function_name, input_data, context, agent, execution):
        ...
        if self.is_langchain_tool(function_name):
            ...
            raise RuntimeError(
                f"Langchain tool {function_name} not found, please install langchain"
            )
        ...
        function_instance = self.setup_function_instance(function_name, function_path, context, agent, execution)
        try:
